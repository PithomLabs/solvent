...
//
// Production embeddings are now real: internal/corpus/embed.go calls Amazon Titan
// Text Embeddings V2. This function survives ONLY as a test fixture, and it lives
// in a _test.go file so that it is structurally impossible to call from production
// code -- the compiler, not a convention, enforces that boundary now.
//
// It was not deleted outright because the corpus suite must stay runnable offline:
// twelve tests need deterministic vectors, and making them depend on Bedrock would
// require network access and AWS credentials to run `task test`. A lexical fixture
// that costs nothing is the right trade for that.
//
// Dim was already 1024 -- Titan v2's default and maximum width -- so the column,
// the index and the query path did not change when the real embedder arrived.
