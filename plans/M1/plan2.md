# M1 Implementation Plan — Kernel Package Compiles — Revision 3

Revision 2 was **✅ APPROVED WITH NITS** (`plans/M1/plan1_review.md`, D-037…D-041:
*"Proceed with implementation. Implement Revision 2 exactly as written. Do not optimize. Do
not redesign. Do not begin M2."*). To be written into the repo as `plans/M1/plan2.md`.

Revision 3 changes **nothing** substantive. It folds in the three documentation/naming nits
and the closing deliverables the review requires. Everything else is Revision 2 verbatim.

## What Revision 3 adds

| Ref | Finding | Change |
|---|---|---|
| **N1** | Mark `SQLCatalog()` in-source so future developers don't start depending on it | Exact comment mandated in §2 and §3: `// Verification support.` / `// Not part of the public kernel API.` |
| **N2** | `m1_<name>` ties the verifier to a milestone it isn't inherently tied to | PREPARE statement names become **`verify_<name>`** (§5 S12) |
| **N3** | `RowsAffected` on `RetractCascade` is inferable but not stated | One sentence added to the §3 contract: it counts **belief rows retracted, not cancelled intents** |
| **Deliverables** | Review requires transcripts at close, not just a report | `scripts/m1_accept.sh` now emits four transcripts to `docs/` (§2, §6) |

Carried forward from the Revision-2 approval, unchanged: **D-032** (scenario-scoped cascade),
**D-033** (`WITH RECURSIVE` canonical, temp table retired), **D-034** (no stub — M1 implements
`RetractCascade`), **D-035** (PREPARE validation), **D-036** (`kernel.Contract`), **D-038**
(`SQLCatalog()` is verification infrastructure, not kernel behavior), **D-039** (PREPARE
validation is a permanent acceptance gate for kernel milestones).
...
| `scripts/m1_accept.sh` | Gate: isolation → build → vet → gofmt → I-7 → PREPARE sweep; emits the four transcripts |
| `docs/M1_REPORT.md` | Closing report |
| `docs/M1_PREPARE.md` | PREPARE transcript — one row per statement |
| `docs/M1_BUILD.md` | Build + vet transcript (both commands, both exit codes) |
| `docs/M1_I7.md` | I-7 report — every write site and the `ExecuteTx` that encloses it |
...
additive — it changes no §4 signature. Per **N1** it carries this exact comment in source, so
nobody starts depending on it:
...
| `RetractCascade(ctx, scenarioID, rootID string) (retracted int, err error)` | **Scenario-scoped (D-032).** In ONE transaction: compute the transitive descendants of `rootID` within `scenarioID`; set all `live` intents on those beliefs to `cancelled`; then set those beliefs from `promoted` to `retracted`. Cancel strictly precedes retract (I-8). Returns the count of beliefs retracted. **`retracted` is the `RowsAffected` of the belief UPDATE only — it counts belief rows retracted, never cancelled intents (N3).** |
...
database — statement names are `verify_` prefixed (N2), because nothing about the verifier is
inherently tied to M1:

```sql
PREPARE verify_<name> AS <statement>;
DEALLOCATE verify_<name>;
```

Report per statement: name, prepared yes/no, SQLSTATE and message on failure, written to
`docs/M1_PREPARE.md`. Non-zero exit on the first failure, printing criterion / observed /
expected in the M0 house style. No transaction is opened; nothing is executed.
...
| S14 | `./scripts/m1_accept.sh` = 0 end to end, and all four transcripts written |
| **M1 overall** | `go build ./...` = 0; `go vet ./...` = 0; `gofmt -l cmd internal` empty; PREPARE sweep green; `./scripts/m1_accept.sh` = 0 |

## Closing deliverables (required by the review)

| Deliverable | Source |
|---|---|
| `docs/M1_REPORT.md` | Written at S15 |
| PREPARE transcript | `docs/M1_PREPARE.md` — all 8 statements, none skipped |
| Build transcript | `docs/M1_BUILD.md` — `go build ./...` |
| Vet transcript | `docs/M1_BUILD.md` — `go vet ./...`, same file, separate section |
| I-7 report | `docs/M1_I7.md` — 6 wrapped write sites, 0 raw writes |
| Remaining blockers | `docs/M1_REPORT.md` §Escalations — E-2 carried items, or "none" |
...
- `docs/M1_I7.md` reports 6 `ExecuteTx`-wrapped write sites and 0 raw writes.
...
- `docs/M1_PREPARE.md` lists all 8 statements prepared, none skipped.
- `docs/M1_BUILD.md` shows both commands at exit 0.

Return to the architect: `M1_REPORT.md`, the PREPARE transcript, the build and vet
transcripts, the I-7 report, and remaining blockers. **Then stop. Do not begin M2.**
