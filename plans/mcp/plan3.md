# Solvent MCP Server — Final Implementation Specification (Phase A)

**Status:** **FINAL — approved for implementation. No further planning cycle.**
The next adversarial signal comes from the coding agent's implementation and execution, not another
theoretical review.

**Supersedes:** `plans/mcp/plan2.md`, as amended by `plans/mcp/plan2_review.md`
(APPROVE after two clarifications — §7.2 and §8.1 below).
**Lineage:** `plan_claude.md` (strategy) → `plan_review.md` → `plan2.md` (locked) →
`plan2_review.md` → **this document**.
**Date:** 2026-08-11
...
This document is self-contained. The coding agent does not need any other file to execute it.
...
**This tool is the agent's only legitimate source of state.** Every count, status, and identifier the
agent asserts must come from here — see §8.1.
...
**Handler rule:** `belief_id` is whatever the caller supplies. In the demo it must be the baseline
UUID **read from `solvent_ledger`**, never a value remembered from the seed step (§8.1).
...
| `solvent_enter_belief` / `create_belief` | The Track 2 baseline is seeded by the existing CLI (§7). MCP demonstrates interaction with a *governed* belief ledger — not an unrestricted notepad for manufacturing beliefs. **Explicitly reaffirmed by `plan2_review.md`.** |
...
No MCP tool creates this state, and none can. That is deliberate — see §3.7.

---

## 7.2 Seed → ingest interaction — **verify, do not assume**

*(Added per `plan2_review.md` finding 1.)*

Act IV calls `solvent_ingest_evidence(track2)` on a database that **already contains the seeded
baseline**. This is a different order from the frozen `task demo:track2`, which ingests first and
enters the baseline second. The end state should be identical, and the analysis below says why — but
the coding agent must **observe** it, not trust it. Criteria 16–17 make it a gate.

### Why it should hold

`pipeline.Run` over `track2/` derives exactly two claims:

| Fixture | Claim | Type | Debt |
|---|---|---|---|
| `release_v350.json` | `"fix available for etcd-io/etcd v3.5.0"` (`internal/derive/derive.go:86`) | `accommodated` | 4 |
| `postmortem_v35.json` | `"v3.5.0–v3.5.2 has documented data inconsistency"` (`internal/derive/derive.go:232`) | `derived` | 6 |

Neither string equals the baseline claim, so `EnsureBelief`'s find-or-create on
`(scenario_id, claim)` creates two **new** beliefs and never touches the baseline. **No duplicate
baseline; the baseline UUID is stable across ingest.**

**One subtlety — do not read this as a bug.** `normalize.ContentSHA256` is computed from *semantic
fields*, not file bytes — for a release, `{version, commit_sha, prerelease}`
(`internal/normalize/normalize.go:348`). So the digest the pipeline computes for `release_v350.json`
differs from the file digest the seed passed via `--evidence-sha`. `evidenceExists` dedups on
`(belief_id, content_sha256)` and **both** fields differ, so both evidence rows survive. The result
is three evidence rows with three distinct digests, two of which carry the same `source_url`
(the v3.5.0 release tag). That is correct and matches the frozen `task demo:track2` end state.

### Expected state immediately after Act IV's ingest, before `solvent_falsify`

| | Expected |
|---|---|
| beliefs | **3** — baseline (`postulated`, `promoted`, debt `{}`), `accommodated` (4 debts, `entered`), `derived` (6 debts, `entered`) |
| baseline UUID | **identical to the seeded UUID** — assert equality explicitly |
| evidence | **3** rows, **3 distinct** `content_sha256`, two sharing one `source_url` |
| intents | **1**, `live`, on the baseline |
| `audit_live_on_nonpromoted` | **0** |
| `belief_edge` | **0** rows |

### Expected state after `solvent_falsify(baseline_uuid)`

| | Expected |
|---|---|
| baseline belief | `retracted` |
| its intent | `cancelled` |
| the other two beliefs | **still `entered`, untouched** |
| `retracted` (tool return) | **1** |
| `audit_live_on_nonpromoted` | **0** |
| `belief_edge` | **0** rows |

The last two rows of that table are the honest, observable meaning of *single-belief retraction*: the
two pipeline-derived beliefs are not descendants of the baseline (there are no edges), so they are
correctly unaffected.

`solvent_falsify` must be called with the baseline UUID **read from `solvent_ledger`** — never one
remembered from the seed command's stdout (§8.1).
...
## 8.1 Grounding rule (binding)

*(Added per `plan2_review.md` finding 2.)*

> **The scripted judge may request an action, but every state, count, status, or identifier the agent
> asserts must be grounded in the immediately preceding tool result.**

Mechanically, this means:

- **Counts come from the ledger.** Act I's "six review obligations" is `len(debt)` from the
  `solvent_ledger` call that immediately precedes it — not from the fixture, not from the seed
  command, not from the tool description. (`solvent_retire_debt`'s description does list the six
  valid item *names*; that is a schema fact, not a claim about current state.)
- **Identifiers come from the ledger.** The baseline `belief_id` passed to `solvent_promote`,
  `solvent_authorize_action`, and `solvent_falsify` is read from `solvent_ledger`, never carried over
  from the seed step's stdout.
- **Statuses come from the ledger.** "It's `entered`, not authorized" and "the intent is `cancelled`"
  must each follow a ledger read, not a recollection of what a mutation returned earlier.
- **Refusal details come from the refusal.** SQLSTATE and constraint name are quoted from the tool
  error result, never from this document.

This is checked in criterion 18 by reading one full rehearsal transcript.
...
*(The count and the status both come from the call just above — §8.1.)*
...
`solvent_authorize_action(scenario: "track2", belief_id: <from ledger>, action: "deploy etcd v3.5.0")` →
...
`solvent_ledger(scenario: "track2")` — re-read to obtain the baseline UUID and confirm it is
unchanged, then:

`solvent_falsify(scenario: "track2", belief_id: <baseline UUID from ledger>)` →
...
`solvent_ledger(scenario: "track2")` → baseline belief `retracted`; its intent `cancelled`; the other
two beliefs still `entered`; audit 0.
...
## 8.2 Script honesty rules (binding)

- The grounding rule in §8.1 applies to every spoken assertion.
...
> Obtain the belief's id from `solvent_ledger` immediately before calling.

**`solvent_ledger`**
> Read the current ledger for a scenario: beliefs with status and open debt, optionally their
> evidence, action intents with state, and the safety audit count. **This is the only source of truth
> about current state.** Call it before asserting any count, status, or identifier, and call it again
> after any mutation — never answer from memory of an earlier tool result, and never state a number
> you did not just read here.
...
Numbered and mechanically checkable. Phase A ships when all eighteen pass.
...
15. `task mcp:seed` reproduces the exact post-seed state in §7.1 from a clean database, and the full
    §8 script runs end to end without manual database intervention.

**Seed → ingest interaction** *(new — `plan2_review.md` finding 1)*

16. **Baseline stability:** the baseline belief UUID recorded immediately after `task mcp:seed` is
    byte-identical to the UUID present after Act IV's `solvent_ingest_evidence`. Exactly one belief
    with the baseline claim exists — no duplicate.
17. **Post-ingest state matches §7.2 exactly:** 3 beliefs with the stated types, statuses, and debt
    counts; 3 evidence rows with 3 distinct `content_sha256`; 1 `live` intent on the baseline;
    audit 0; `belief_edge` 0. And after `solvent_falsify`: baseline `retracted`, its intent
    `cancelled`, **the other two beliefs still `entered` and untouched**, `retracted` = 1, audit 0.

**Agent grounding** *(new — `plan2_review.md` finding 2)*

18. In one full rehearsal transcript, every count, status, and UUID the agent asserts is traceable to
    the immediately preceding tool result. Specifically: the "six obligations" count follows a
    `solvent_ledger` call, and the `belief_id` passed to `solvent_falsify` was read from
    `solvent_ledger` rather than carried from the seed step.
...
| MCP belief creation (`enter_belief` / `create_belief`) | Reaffirmed by `plan2_review.md`: MCP demonstrates a *governed* ledger, not an agent notepad. |
...
Only after all eighteen criteria pass.
...
4. **90-second screen recording** of the full §8 conversation — the fallback if a judge's client
   won't connect.
5. **Two timed rehearsals.** One of these transcripts satisfies criterion 18.
...
# Appendix A — `plan2_review.md` coverage

| Review item | Resolution | Section |
|---|---|---|
| **Fix 1** — clarify seed → ingest; verify rather than assume; make it a concrete acceptance check | New §7.2 with the code-level reason it should hold plus exact expected state before and after falsify; baseline-UUID stability and `solvent_falsify` targeting stated explicitly | §7.2, §8 Act IV, criteria **16–17** |
| **Fix 2** — tighten "every number comes from a tool result" into a mechanical rule | New §8.1 grounding rule in the review's own words, applied to counts, identifiers, statuses, and refusal details; `solvent_ledger` description strengthened | §8.1, §8.2, §9, criterion **18** |
| Do **not** add `create_belief` / `enter_belief` | Reaffirmed and given its reason in the absent-tools table and the out-of-scope fence | §3.7, §11 |
| Keep `source_observed_at`, `belief_edge`, kernel changes, HTTP, auth, client matrices out | Unchanged | §5.3, §11 |
| No further Claude planning cycle | This document is marked FINAL; next signal is implementation | header |

# Appendix B — `plan_review.md` coverage (carried forward)

| Review item | Resolution | Section |
|---|---|---|
| Don't depend on the LLM being "stupid"; make the judge prompt explicit | Scripted judge prompt; descriptions honest and instructing the agent **not to pre-check** | §8 Act I, §9 |
| Audit envelope on state-changing tools only; never computed in MCP | `{result, audit:{…}}` on the five mutating tools; ledger carries it in-snapshot; always from `pipeline.AuditIntent` | §4.1, criterion 7 |
| Simplify the demo | Four acts, single Track 2 narrative | §8 |
| Prove the agent is not the security boundary | Scripted beat + trust-boundary diagram, also in the README | §2, §8 Act II, §12.2 |
| Verify no user-controlled fixture path | No path parameter anywhere; enum → server-owned root; startup validation | §5, criteria 8–10 |
