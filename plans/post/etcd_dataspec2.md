# etcd Real-Data Implementation Specification (v2)

**Decision Date:** 2026-08-10
**Status:** CORRECTED — fixes 4 inconsistencies from review; architecture unchanged
**Frozen Architecture:** Waves 1–4 — no schema changes, no new kernel functions, no new tables
**Source:** Revision of `etcd_real_data_implementation_spec.md` per `etcd_dataspec_review.md`
...
| # | Blocker | Fix |
|---|---|---|
| 1 | Replay not idempotent for intents | Intent creation is an explicit demo action, not a pipeline step |
| 2 | Track 2 uses same fixture directory as Track 1 | Separate `track1/` and `track2/` subdirectories |
| 3 | `maintainer_review.json` in ingested directory | Removed entirely (unused raw reference) |
| 4 | Evidence count "6" is wrong | Corrected to 3 |
...
- Five real fixture files across two track directories + manifest + attribution
...
| # | File | Track | Real / Operator | Source Type | Purpose |
|---|---|---|---|---|---|
| 1 | `ghsa_advisory.json` | 1 | Real | `github_advisory` (NEW) | Derives vulnerability claim; retires `needMap` |
| 2 | `release_v3527.json` | 1 | Real | `release` (existing) | Pins affected deployment target as dated fact |
| 3 | `release_v3528.json` | 1 | Real | `release` (existing) | Fix-availability evidence; retires `needMap`, `needInvariant` |
| 4 | Operator review | 1 | Operator-asserted | Demo script (kernel calls) | Retires remaining debts for vulnerability belief |
| 5 | `release_v350.json` | 2 | Real | `release` (existing) | Supporting evidence for baseline deployment decision |
| 6 | `postmortem_v35.json` | 2 | Real | `postmortem` (NEW) | Invalidating evidence for deployment decision |
| — | Belief entry | 2 | Not a record | `kernel.EnterBelief` | "etcd v3.5.0 is approved for production deployment (decision as of 2021-06-16)" |

### New source types vs fixture files

- **2 new source types**: `github_advisory`, `postmortem`
- **5 fixture files**: 3 in `track1/`, 2 in `track2/`
- 3 fixture files use existing source types (`release`)
...
Records are processed in directory sort order (alphabetical by filename within `track1/`):
...
All three promotion attempts fail with SQLSTATE 23514 (ErrPromotionBlocked). **No intents are created by the pipeline.**
...
**Step 6 — Intent (explicit demo action, NOT a pipeline step):**
- `IntentOnPromoted(scenarioID, belief_id_A, "deploy etcd v3.5.28")` → SUCCESS
- Intent state = 'live'

**Step 7 — Replay (determinism check):**
- Re-run pipeline → `EnsureBelief` dedups, `AddEvidence` dedups (UNIQUE hash) → zero new evidence/belief rows
- `DebtItemsForEvidence` idempotent, `RetireDebt` idempotent (array_remove on absent item)
- `Promote` idempotent (already promoted)
- Pipeline does NOT call `IntentOnPromoted` — no duplicate intents
- Transcript output identical to first run → `VERDICT: PASS`

### Demo beats

1. Pipeline runs, 3 beliefs entered, 3 evidence records attached
...
5. Intent created as explicit demo action → show intent
6. Replay → zero new evidence/belief rows → show determinism
...
**Step 6 — Postmortem ingested (via pipeline on `track2/` fixtures):**
...
internal/derive/testdata/etcd_real/
  track1/
    ...
    release_v3527.json            # release (existing source type)
    release_v3528.json            # release (existing source type)
  track2/
    release_v350.json             # release (existing source type)
...
- `manifest.json`: `{"files": {"track1/ghsa_advisory.json": "sha256:...", ...}}` — computed after all fixtures written
- `ATTRIBUTION.md`: licensing and source notices for every record
- `manifest.json` and `ATTRIBUTION.md` are at the `etcd_real/` root — NOT in `track1/` or `track2/`
- The pipeline is invoked with `--fixtures internal/derive/testdata/etcd_real/track1` or `track2`
...
### Intent is NOT a pipeline step

The pipeline (`belief.Process`) does NOT call `IntentOnPromoted`. Intent creation is an explicit demo action performed after the pipeline run and operator review. This separation ensures:
- Pipeline replay is deterministic (zero new evidence/belief rows)
- Intent creation is a single explicit action, not duplicated on replay
...
| `internal/pipeline/pipeline.go` | Add 5 entries to `sourceTypeMap` (5 fixture filenames across 2 directories) | ~8 |
...
### Fixture files (5 total, 2 new source types)

| File | Source type | New / Existing | Directory |
|---|---|---|---|
| `ghsa_advisory.json` | `github_advisory` | NEW | `track1/` |
| `release_v3527.json` | `release` | existing | `track1/` |
| `release_v3528.json` | `release` | existing | `track1/` |
| `release_v350.json` | `release` | existing | `track2/` |
| `postmortem_v35.json` | `postmortem` | NEW | `track2/` |
...
# Step 1: Pipeline — processes track1/ fixtures only
solvent --dsn "$FABLE_DSN" --scenario 00000000-0000-0000-0000-000000000001 --reset \
  --fixtures internal/derive/testdata/etcd_real/track1 \
  --out /tmp/track1_transcript.json

# Expected: 3 beliefs entered, 3 evidence attached, 3 promotions REFUSED, 0 intents

# Step 2: Operator review — retire remaining debts on vulnerability belief
...
# Step 3: Promotion + intent (explicit demo actions)
# Via operator-review script (promotes after retiring debts)
# Or via direct kernel calls from demo script

# Step 4: Replay — verify determinism
solvent --dsn "$FABLE_DSN" --scenario 00000000-0000-0000-0000-000000000001 \
  --fixtures internal/derive/testdata/etcd_real/track1 \
  --out /tmp/track1_rerun.json

# Expected: zero new evidence/belief rows, same transcript → VERDICT: PASS
```

### Track 2 — Historical retraction

```bash
# Step 1: Pipeline — processes track2/ fixtures only
solvent --dsn "$FABLE_DSN" --scenario 00000000-0000-0000-0000-000000000002 --reset \
  --fixtures internal/derive/testdata/etcd_real/track2 \
  --out /tmp/track2_transcript.json

# Step 2: Demo script — enter baseline belief, attach evidence, retire debts, promote, create intent
# (handled by demo script or direct kernel calls — not pipeline)

# Step 3: Falsify — retraction
...
### Expected transcript output (Track 1, pipeline only)
...
### Intent creation semantics

- Pipeline NEVER calls `IntentOnPromoted` — `intents_created: 0` in all transcript output
- Intent is created as an explicit demo action AFTER pipeline + operator review
- On replay, pipeline produces identical transcript; intent is NOT re-created
- This separation makes replay deterministic without kernel changes
...
- [ ] Pipeline processes all 3 track1/ fixtures without error
- [ ] 3 beliefs entered with correct claim strings
- [ ] 3 evidence records attached with correct provenance (`external_feed`)
...
- [ ] Intent created as explicit demo action (NOT by pipeline)
- [ ] Replay produces zero new evidence/belief rows (determinism)
...
- [ ] `sourceTypeMap` includes all 5 fixture filenames across `track1/` and `track2/`
...
- [ ] Pipeline never calls `IntentOnPromoted`
- [ ] Track 1 and Track 2 use separate fixture directories
...
1. plans/post/etcd_dataspec2.md (this document)
...
10. Replay idempotent: pipeline re-run produces zero new evidence/belief rows
11. Intent not created by pipeline: IntentOnPromoted never called from belief.Process
12. RetractCascade order: cancel before retract (I-8)
13. Audit = 0 after every operation
14. No fabricated URLs: all source_url values are real
15. Provenance honest: operator_asserted only outside pipeline
16. Separate fixture directories: track1/ and track2/ are never mixed
...
**Awaiting user decision on which option to implement before writing production code.**
