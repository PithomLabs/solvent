...
Two new source types (`github_advisory`, `postmortem`) added to the normalize→derive→mapping chain. Five real fixture files created across two track directories. Pipeline Phase 3.5 (intent proposal) removed per spec. Standalone `cmd/operator-review` script created (Option B).
...
| `internal/normalize` tests | PASS |
| `internal/derive` tests (16) | PASS |
| DB-dependent tests (belief, intent, kernel, operator review) | PASS |
| `internal/pipeline` tests | PASS (9/9) |
...
| §10 Code Changes | 6 files modified, 1 new file, 5 fixture files |
...
|---|---|---|
| `internal/normalize` | 9 | PASS |
| `internal/derive` | 16 | PASS |
| `internal/pipeline` | 9 | PASS |
| `internal/belief` | 17 | PASS |
| `internal/intent` | 3 | PASS |
| `internal/kernel` | 24 | PASS (4 new: OR-1 through OR-4) |
| **Total** | **78** | **ALL PASS** |
